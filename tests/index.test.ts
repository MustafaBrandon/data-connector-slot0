...
describe("WASM Transformation Module", () => {
  ...
  // });
    ...
    test("can process the final response and return true for callback termination", async () => {
      ...
      myModule = await loadWasm(fs.readFileSync("/home/rakesh/SteerProtocol/data-connectors/data-connector-slot0-algebra/build/release.wasm"), {})

      const _response = '[{"type": "BigNumber","hex":"0x0c32ec1d2f03950282d5"},-282698,587,57987,100,100,true]'
      ...
      console.log("🚀 ~ file: index.test.ts:91 ~ test ~ result:", result)

      const transformResult = myModule.transform();
      console.log("🚀 ~ file: index.test.ts:94 ~ test ~ transformResult:", transformResult)
